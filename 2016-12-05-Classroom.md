...
The original goal for the project, after completing the value steam mapping, was to create a github repository that students and teachers could pull from to deploy Virtual Machines to Azure to take advantage of DevOps practices by simplifying and automating everyting while splitting the tasks out to standalone small tasks that could easily be run in it's own environment.
...
## Infrastructure as Code ###
For this project we actually provided many different ways to accomplish the same task. This was a requirement because the primary TA is graduating soon and he wanted to setup his predicessor for success.  It also gives the advantage of making different code available for students to use in their platform or language of choice.  The fundamental deployments that were required included: 
...
We used many DevOps capabilities to deliver a fully automated solution to stand up a class of hundreds of students, many TA as well as muliple professors.  The required expectation of being able to use different languages to support different options of the people that would in the future be doing it made it a bit more challenging.  However the result and open nature of the code can be used by not only any university but any classroom environment.  We see this being used for one day or multiple day workshops, classes that last weeks, months or even years.  It can be used to setup any demo or teaching environment where simplicity of deployment and consistancy of process are required. 
